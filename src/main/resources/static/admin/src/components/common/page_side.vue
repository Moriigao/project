<template>
  <el-menu class="side el-menu-vertical-demo" default-active="/" @open="handleOpen" @close="handleClose"
    ...
    <el-menu-item index="/">
     ...
     <el-submenu index="user" v-show="user_group == '管理员' || $check_group(['/user/table','/user_group/table','/ordinary_users/table'])">
    ...
    <el-submenu index="web" v-show="user_group == '管理员' || $check_group(['/nav/table','/link/table','/slides/table','/ad/table','/sensitive_vocabulary/table'])">
      ...
      <el-menu-item v-show="user_group == '管理员' || $check_action('/sensitive_vocabulary/table')" index="/sensitive_vocabulary/table" @click="handleButtonClick(1)">
        <span>敏感词管理</span>
      </el-menu-item>
    </el-submenu>

    <el-submenu index="webs" v-show="$check_action('/message/table')">
      ...
      <el-menu-item v-show="$check_action('/message/table')" index="/message/table" @click="handleButtonClick(10)">
        <span>{{$page_title("/message/table") || 留言反馈}}</span>
      </el-menu-item>
    </el-submenu>

    <el-submenu index="notic" v-show="$check_group(['/notice/table'])">
      <template slot="title"><i class="el-icon-warning-outline"></i><span ref="customTemplate2">通知公告管理</span></template>
      <el-menu-item v-show="$check_action('/notice/table')" index="/notice/table" @click="handleButtonClick(2)">
        <span>通知公告</span>
      </el-menu-item>
    </el-submenu>

    <el-submenu index="news" v-show="$check_group(['/article/table','/article_type/table','/exam/table','/evaluation/table'])">
      <template slot="title"><i class="el-icon-s-opportunity"></i><span ref="customTemplate3">资讯管理</span></template>
      <el-menu-item v-show="$check_action('/article/table')" index="/article/table" @click="handleButtonClick(3)">
        <span>{{$page_title("/article/table") || "资讯"}}</span>
      </el-menu-item>
      <el-menu-item v-show="$check_action('/article_type/table')" index="/article_type/table" @click="handleButtonClick(3)">
        <span>{{$page_title("/article_type/table") || "资讯分类"}}</span>
      </el-menu-item>
    </el-submenu>

    <el-submenu index="content" v-show="$check_group(['/forum/table','/forum_type/table'])">
      <template slot="title"><i class="el-icon-chat-line-round"></i><span ref="customTemplate6">论坛管理</span></template>
  ...
  export default {
    ...
    computed: {
      auth() {
        ...
        let site_list = []
		    let map_list = []
        let list = ["exam", 'evaluation', "article", "article_type", "comment", "forum", "forum_type", "notice", "message"];
