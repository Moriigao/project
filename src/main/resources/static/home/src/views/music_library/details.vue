<template>
	<div class="diy_details page_music_library" id="music_library_details">
		<div class='warp_dt_box'>
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_music_library'>
							<div_music_library :obj="obj"></div_music_library>
						</div>
					</div>
				</div>
			</div>
				...
				<div class="row review">
					...
					<div class="col">
						...
						<div class="comment_box"	v-if="list_comment.length">
							<list_comment class="w-100" :list="list_comment" :obj="form_comment" @reset="update_comment()"/>
							<b-pagination-nav :base-url="$route.path" :number-of-pages="count_pages" :link-gen="link_gen" @change="change_page" first-number last-number/>
						</div>
						<!-- 发表评论 -->
						<div class="form_editor form_edit_box">
							...
							<div class="fr mr-3" v-show="form_comment.reply_to_id">
								<b-button variant="outline-primary reset_answer_personnel" size="sm" @click="form_comment.reply_to_id = 0">重置回复人</b-button>
							</div>
...
<script>
	import div_music_library from "@/components/diy/div_music_library.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/music_library/set?music_library_id=' + obj["music_library_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.music_library_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "music_library",
					source_field: "music_library_id",
					source_id: obj["music_library_id"],
					user_id
				};
