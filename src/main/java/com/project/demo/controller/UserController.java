...
public class UserController extends BaseController<User, UserService> {
    ...
    public Map<String, Object> login(@RequestBody Map<String, String> data, HttpServletRequest httpServletRequest) {
        ...
        try {
            password = RsaUtils.decryptByPrivateKey(password);  //前端使用jsencrypt进行RSA加密
        }catch (Exception e){
    ...
    public Integer tokenGetUserId(String token) {
        ...
        AccessToken byToken = (AccessToken) redisTemplate.opsForValue().get(token); //从 Redis 中查找与 token 对应的值
        if(byToken == null){
            return 0;
        }
